...
struct WeekdayTotal: Identifiable, Hashable, Sendable {
    let weekday: Int
    let total: Decimal
...
struct MonthlyCategoryTotal: Identifiable, Hashable, Sendable {
    let monthStart: Date
    let categoryId: UUID
    let categoryName: String
    let icon: CategoryIcon?
    ...
    var id: String { "\(monthStart.timeIntervalSince1970)-\(categoryId.uuidString)" }
}
...
struct MonthlyKindTotal: Identifiable, Hashable, Sendable {
    let monthStart: Date
    let income: Decimal
    let expense: Decimal
